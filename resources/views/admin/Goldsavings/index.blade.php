...
@section('content')
    <div class="content">
        ...
        <div class="gold-reg-container">
            <!-- Search Bar -->

            <div class="d-flex justify-content-between mb-3">
                <h3 class="me-3">Gold Savings Customer Details</h3>
                <form action="{{ route('admin_gold') }}" method="GET" class="d-flex" id="searchForm">
            ...
            <table >
                ...
                <tbody>
                    @foreach ($golddetails as $key=>$golddetail)
                    <tr>
                        <td>{{ $key+1 }}</td>
                        <td>{{ $golddetail->name }}</td>
                        <td>{{ $golddetail->phone }}</td>
                        <td>{{ $golddetail->email }}</td>
                        <td>{{ $golddetail->address }}</td>
                        <td style="text-align: center">
                            <a href="{{ route('admin_gold_transaction', ['id' => $golddetail->user_id]) }}" title="View" class="no-link-style">
                                <i class="fas fa-credit-card"></i>
                            </a>
                        </td>
                        <td>
                            <a href="{{ route('admin_gold_view', ['id' => $golddetail->id]) }}" title="View" class="no-link-style">
                                <i class="fas fa-eye"></i>
                            </a>
                            <button data-toggle="modal" data-target="#deleteModal" data-id="{{ $golddetail->id }}" id="delete_customer" class="btn no-outline">
            ...
            <div class="d-flex mt-4 flex-wrap align-items-center">
                <p class="text-muted mb-sm-0">Showing {{ $golddetails->firstItem() }} to {{ $golddetails->lastItem() }} of {{ $golddetails->total() }} entries</p>
                <nav class="ms-auto">
                    {{ $golddetails->links('pagination::bootstrap-4') }} <!-- Use 'pagination::bootstrap-4' or adjust as per your pagination style -->
                </nav>
            </div>
        </div>
    </div>
  ...
  <div class="modal-dialog modal-dialog-centered" role="document">
    <div class="modal-content">
      ...
      <div class="modal-footer deletecustomer">
        <form id="deleteForm" method="POST" action="{{ route('admin_gold_destroy') }}">
          @csrf
...
<script>
    $('#deleteModal').on('show.bs.modal', function (event) {
        ...
        var route = "{{ route('admin_gold_destroy', ['id' => 'ID_PLACEHOLDER']) }}";
