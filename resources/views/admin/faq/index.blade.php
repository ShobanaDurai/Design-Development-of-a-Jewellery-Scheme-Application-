    ...
    <div class="content">
        ...
        <div class="gold-reg-container">
            ...
            <div class="d-flex justify-content-between mb-3">
                ...
                <form action="{{ route('faq.index') }}" method="GET" class="d-flex" id="searchForm">
                ...
                <a href="{{ route('faq.create') }}" class="btn  mb-2 mr-2" style="background-color: #45A049;color:#fff" >
                    Add New
                </a>
            </div>
                    ...
                    @foreach($faq as $key=> $faqs)
                        <tr>
                            ...
                            <td>
                                ...
                                <a href="{{ route('faq.edit',encrypt($faqs->id))}}" title="Edit" class="no-link-style">
                                    <i class="fas fa-edit"></i>
                                </a>
                                <button data-toggle="modal" data-target="#deleteModal" data-id="{{ $faqs->id }}" id="delete_customer" class="btn no-outline">
            ...
            <div class="d-flex mt-4 flex-wrap align-items-center">
                <p class="text-muted mb-sm-0">Showing {{ $faq->firstItem() }} to {{ $faq->lastItem() }} of {{ $faq->total() }} entries</p>
                <nav class="ms-auto">
                    {{ $faq->links('pagination::bootstrap-4') }} <!-- Use 'pagination::bootstrap-4' or adjust as per your pagination style -->
                </nav>
            </div>
        </div>
    </div>
  ...
  <div class="modal-dialog modal-dialog-centered" role="document">
    <div class="modal-content">
      ...
      <div class="modal-footer deletecustomer">
        <form id="deleteForm" method="POST" action="{{ route('faq.destroy') }}">
          @csrf
...
<script>
    $('#deleteModal').on('show.bs.modal', function (event) {
        ...
        var route = "{{ route('faq.destroy', ['id' => 'ID_PLACEHOLDER']) }}";
